...
A friend of mine writes a blog about flags. He set up a system where typing `/write-post Poland` produces a full draft in his voice. Here's how the process maps:

![Blog post process — one agent, multiple hats](diagrams/diagram-blog-process.svg)

The command file is the orchestrator — it defines the steps. The agent checks an archive of old posts (explorer), writes the draft using a voice skill (actor), and presents it for the author's review (reviewer). If the draft isn't right, the author gives feedback and the agent revises. After a few posts, the author noticed the agent kept missing adoption dates — he added a rule to fix it. The process improved.
...
One command. Four reference files. A single agent wearing multiple hats. Chapter 7 walks through the full setup — how the archive was built, how the voice skill was created, and how to build this kind of system for your own work.
